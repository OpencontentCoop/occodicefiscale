...
$script = eZScript::instance(array(
    'description' => ("Converte un attributo ezstring in occodicefiscale"),
    'use-session' => false,
    'use-modules' => true,
    'use-extensions' => true
));
...
if ($class instanceof eZContentClass) {
    ...
    if (isset($attributes[$options['attribute']])) {
        ...
        if($attributeClass instanceof eZContentClassAttribute){
            if ($attributeClass->attribute('data_type_string') == 'ezstring'){
                ...
                $db->begin();
                $attributeClass->setAttribute('data_type_string',OCCodiceFiscaleType::DATA_TYPE_STRING);
                $attributeClass->store();
                foreach($attributeObjects as $attributeObject){
                    $attributeObject->setAttribute('data_type_string',OCCodiceFiscaleType::DATA_TYPE_STRING);
